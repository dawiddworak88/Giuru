...
namespace Seller.Web.Areas.Dashboard.ModelBuilders
{
    public class ProductsSalesAnalyticsModelBuilder : IAsyncComponentModelBuilder<ComponentModelBase, ProductsSalesAnalyticsViewModel>
    {
        ...
        {
            var fromDate = DateTime.UtcNow.AddDays(DashboardConstants.ProductsAnalyticsDifferenceInDays);
            var toDate = DateTime.UtcNow;

            var productsSales = await _salesAnalyticsRepository.GetProductsSales(componentModel.Token, componentModel.Language, fromDate, toDate, DashboardConstants.DefaultProductsSalesSize, $"{nameof(ProductSalesApiItem.ProductName)} desc");
