...
namespace Seller.Web.Areas.Clients.Repositories.ClientTeamMembers
{
    ...
    {
        Task<PagedResults<IEnumerable<TeamMember>>> GetAsync(string token, string language, Guid? organisationId, string searchTerm, int pageIndex, int itemsPerPage, string orderBy);
