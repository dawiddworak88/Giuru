...
namespace Seller.Web.Areas.Products.Repositories
{
    public interface IProductAttributesRepository
    {
        ...
        Task<PagedResults<IEnumerable<ProductAttribute>>> GetProductAttributesAsync(string token, string language, string searchTerm, Guid? sellerId, int pageIndex, int itemsPerPage, string orderBy);
