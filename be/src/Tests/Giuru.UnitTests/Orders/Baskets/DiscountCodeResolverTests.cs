...
{
    /// <summary>
    /// The buyer and the seller share one copy of the resolver, so this matrix is the single
    /// specification for both.
    /// </summary>
    public class DiscountCodeResolverTests
    {
        ...
        {
            var resolution = DiscountCodeResolver.Resolve(hasDiscountCode, requestedDiscountCode, persistedDiscountCode, hasItems);
        ...
        public void RequiresPersistedDiscountCode_OnlyAsksForTheBasketWhenTheRequestCannotResolveOnItsOwn(
            ...
            bool expected)
        {
            Assert.Equal(expected, DiscountCodeResolver.RequiresPersistedDiscountCode(hasDiscountCode, requestedDiscountCode, hasItems));
        }
        ...
        public void Resolve_WhenThePersistedCodeIsNotRequired_IgnoresIt(
            ...
            bool hasItems)
        {
            if (DiscountCodeResolver.RequiresPersistedDiscountCode(hasDiscountCode, requestedDiscountCode, hasItems))
            {
                return;
            }
            ...
            var withoutPersisted = DiscountCodeResolver.Resolve(hasDiscountCode, requestedDiscountCode, null, hasItems);

            foreach (var persisted in new List<string> { null, "", "   ", "SUMMER25", "WINTER25", persistedDiscountCode })
            {
                var withPersisted = DiscountCodeResolver.Resolve(hasDiscountCode, requestedDiscountCode, persisted, hasItems);
